...
def _recon_assessment() -> dict[str, Any]:
    return {
        ...
        "cve_findings": [
            {"service": "http", "product": "nginx", "version": "1.18.0", "count": 2, "cves": ["CVE-2023-44487", "CVE-2021-23017"]},
            {"service": "http-proxy", "product": "express", "version": "4.18.2", "count": 1, "cves": ["CVE-2024-27298"]},
        ],
...
def _enhanced_report() -> dict[str, Any]:
    ...
    findings_spec: list[tuple[str, str, float, str, str]] = [
        ...
        ("Unauthenticated Template Injection → RCE on edge-web-01", "Critical", 9.8, "edge-web-01 (198.51.100.23)", "Server-Side Template Injection"),
        ("Insecure Deserialization in Meridian API (api-01)", "Critical", 9.4, "api-01 (10.42.10.31)", "Insecure Deserialization"),
        ("Domain Admin credential reuse via svc-web on identity-01", "Critical", 9.1, "identity-01 (10.42.30.10)", "Credential Exposure"),
        ("Stored XSS in Portal comment field → session hijack", "High", 8.2, "edge-web-01 (198.51.100.23)", "Cross-Site Scripting"),
        ("IDOR on /api/files/{id} exposes neighboring tenants", "High", 7.9, "api-01 (10.42.10.31)", "Insecure Direct Object Reference"),
        ("Service identity svc-web has excessive read on app-01", "High", 7.6, "app-01 (10.42.10.21)", "Privilege Misconfiguration"),
        ("Privilege escalation via SUID helper on app-01", "High", 7.8, "app-01 (10.42.10.21)", "Privilege Escalation"),
        ("Lateral movement app-01 → api-01 via cached service token", "High", 7.5, "api-01 (10.42.10.31)", "Lateral Movement"),
        ("SMB share readable on files-01 (anonymous read)", "High", 7.3, "files-01 (10.42.20.15)", "Share Misconfiguration"),
        ("Weak password policy on identity-01 (svc-backup)", "Medium", 6.4, "identity-01 (10.42.30.10)", "Weak Credentials"),
        ("Backup archive world-readable on backup-01", "Medium", 6.1, "backup-01 (10.42.30.20)", "Information Disclosure"),
        ("Exposed .git directory on edge-web-01", "Medium", 5.8, "edge-web-01 (198.51.100.23)", "Information Disclosure"),
        ("Missing security headers on portal (CSP, HSTS)", "Medium", 5.3, "edge-web-01 (198.51.100.23)", "Security Misconfiguration"),
        ("Verbose error messages leak stack traces on api-01", "Medium", 5.0, "api-01 (10.42.10.31)", "Information Disclosure"),
        ("Workstation-07 unpatched SMB (synthetic candidate)", "Low", 3.9, "workstation-07 (10.42.20.25)", "Patch Management"),
        ("Cache-01 Redis without AUTH (lab-only binding)", "Low", 3.7, "cache-01 (10.42.10.40)", "Misconfiguration"),
        ("Monitor-01 Grafana default creds present (lab)", "Low", 3.5, "monitor-01 (10.42.30.30)", "Default Credentials"),
        ("DB-01 verbose logging exposes query timing (lab)", "Low", 2.8, "db-01 (10.42.20.16)", "Information Disclosure"),
    ]
    ...
    timeline: list[dict[str, Any]] = [
        {"timestamp": _iso(0), "event_type": "run_started", "description": "Run started — target portal.meridian-lab.example (synthetic lab)", "target": "portal.meridian-lab.example", "module": "orchestrator", "result": "success"},
        {"timestamp": _iso(18), "event_type": "target_resolved", "description": "Target resolved to 198.51.100.23 (edge-web-01)", "target": "edge-web-01", "module": "dns", "result": "success"},
        {"timestamp": _iso(44), "event_type": "recon_completed", "description": "Reconnaissance completed — 4 open ports, 4 services, OS Linux", "target": "edge-web-01", "module": "recon", "result": "success"},
        {"timestamp": _iso(82), "event_type": "service_identified", "description": "Web service identified: Meridian Portal (Node.js/Express) on :8080", "target": "edge-web-01", "module": "service_enumeration", "result": "success"},
        {"timestamp": _iso(128), "event_type": "weakness_identified", "description": "Potential SSTI weakness identified on edge-web-01 (synthetic candidate)", "target": "edge-web-01", "module": "vuln_research", "result": "success"},
        {"timestamp": _iso(196), "event_type": "initial_access", "description": "Initial access hypothesis confirmed — application tier reached (app-01)", "target": "app-01", "module": "validation", "result": "success"},
        {"timestamp": _iso(243), "event_type": "credential_discovered", "description": "Service identity discovered: svc-web (app-01 → identity-01)", "target": "identity-01", "module": "credential_harvest", "result": "success"},
        {"timestamp": _iso(311), "event_type": "privilege_escalation", "description": "Privilege path identified — SUID helper on app-01 → root context (synthetic)", "target": "app-01", "module": "privesc", "result": "success"},
        {"timestamp": _iso(367), "event_type": "lateral_branch", "description": "First lateral branch attempted: app-01 → workstation-07", "target": "workstation-07", "module": "lateral", "result": "attempt"},
        {"timestamp": _iso(402), "event_type": "branch_blocked", "description": "Branch blocked — insufficient privilege for workstation-07 (expected demo branch failure)", "target": "workstation-07", "module": "lateral", "result": "failure"},
        {"timestamp": _iso(435), "event_type": "alternate_path", "description": "Alternate path selected: app-01 → api-01 via cached service token", "target": "api-01", "module": "lateral", "result": "success"},
        {"timestamp": _iso(511), "event_type": "internal_service", "description": "Internal service reached: api-01 → files share enumeration", "target": "api-01", "module": "enumeration", "result": "success"},
        {"timestamp": _iso(588), "event_type": "file_server_discovered", "description": "File server discovered: files-01 (SMB anonymous read, synthetic)", "target": "files-01", "module": "discovery", "result": "success"},
        {"timestamp": _iso(656), "event_type": "objective_confirmed", "description": "Objective access confirmed — synthetic sensitive-data repository on files-01 (no data extracted)", "target": "files-01", "module": "objective", "result": "success"},
        {"timestamp": _iso(702), "event_type": "assessment_completed", "description": "Assessment completed — 11 assets discovered, 7 reached, 18 findings, 93/100 risk", "target": "portal.meridian-lab.example", "module": "reporting", "result": "success"},
    ]
...
def _events() -> list[dict[str, Any]]:
    ...
    offsets_and_types: list[tuple[int, str, dict[str, Any]]] = [
        ...
        (196, "tool_request", {"name": "template_rce_synthetic", "action": 6, "phase": "validation", "arguments": {"target": DEMO_RESOLVED_IP}}),
        (196, "tool_start", {"name": "template_rce_synthetic", "action": 6, "phase": "validation"}),
        (197, "tool_result", {"name": "template_rce_synthetic", "action": 6, "phase": "validation", "success": True, "exit_code": 0}),
        ...
        (367, "tool_request", {"name": "psexec_svc-web", "action": 11, "phase": "validation", "arguments": {"target": "10.42.20.25"}}),
        (402, "tool_result", {"name": "psexec_svc-web", "action": 11, "phase": "validation", "success": False, "exit_code": 1}),
        (435, "tool_result", {"name": "token_reuse_api", "action": 12, "phase": "validation", "success": True, "exit_code": 0}),
        ...
        (702, "completion", {"state": "completed", "result": {"outcome_summary": "Compromise demonstrated (synthetic)"}}),
