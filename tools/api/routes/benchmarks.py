...
existing route conventions (module-level ``configure``, bearer auth, stable
error shape).
"""
...
_STORAGE: BenchmarkStorage | None = None
_CONFIG: dict[str, Any] = {}


def configure(auth: BearerAuth, service: BenchmarkService, storage: BenchmarkStorage, config: dict[str, Any]) -> None:
    global _AUTH, _SERVICE, _STORAGE, _CONFIG
    _AUTH = auth
    _SERVICE = service
    _STORAGE = storage
    _CONFIG = config
...
def _storage() -> BenchmarkStorage:
    if _STORAGE is None:
        raise HTTPException(status_code=503, detail="Benchmark storage unavailable")
    return _STORAGE
...
def _resolve_run(run_id: str) -> tuple[str, dict[str, Any]]:
    ...
    for suite in storage.list_suites():
        run = storage.load_run(suite, run_id)
        if run is not None:
            return suite, run
...
async def stream_run_events(
    ...
    async def _gen():
        ...
        try:
            ...
            idle = 0
            while True:
                events = storage.load_events(suite, run_id, after=cursor, limit=500)
                for event in events:
...
async def cancel_run(run_id: str, auth: str = Depends(_require_auth)) -> dict[str, Any]:
    service = _service()
    if service.active_run_id != run_id:
...
def _baseline_path() -> Path:
    benchmark_cfg = _CONFIG.get("benchmark", {}) or {}
