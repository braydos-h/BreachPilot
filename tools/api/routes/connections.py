...
def create_router(auth: BearerAuth, config: dict[str, Any], config_path: Path) -> APIRouter:
    """Create a connections router with isolated dependencies."""
    ...
    def _workspace() -> Path:
        ...
        oc_cfg = config.get("operator_connection", {}) or {}
        ws_raw = oc_cfg.get("workspace_dir") or ""
        if not ws_raw:
            # Fallback to exploit workspace_dir
            ws_raw = (config.get("exploit", {}) or {}).get("workspace_dir", "exploit_workspace")
        p = Path(str(ws_raw))
