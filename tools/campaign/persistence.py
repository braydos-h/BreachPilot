...
class PersistenceMixin:
    ...
    async def _phase_persistence(self, state: AttackState) -> None:
        ...
        if not getattr(self, "_tool_executor", None):
            state.add_timeline_event("persistence_skipped", "No tool_executor wired -- persistence scripts not dispatched")
        ...
        for mod_name in mod_names:
            ...
            if not script:
                ...
                continue
            task = AttackTask(task_id=self._new_task_id(), phase=AttackPhase.PERSISTENCE, module_name=mod_name, target=state.target, aggression=state.aggression, priority=60)  # type: ignore[attr-defined]
            ...
            if marker:
                ...
                state.add_timeline_event("persistence_established", f"{mod_name} installed persistence via {marker}", {"module": mod_name, "method": marker})
            else:
                from tools.campaign.state import TaskStatus
                task.status = TaskStatus.FAILED
                task.error = "no PERSISTENCE_INSTALLED marker in dispatch output"
                state.add_timeline_event("persistence_failed", f"{mod_name} dispatch did not confirm persistence", {"module": mod_name})


class AutonomousOrchestrator(PlannerMixin, PersistenceMixin):
    _max_module_failures: int = 3

    def __init__(self, mission_config: dict[str, Any], workspace_root: Path, tool_executor: Callable[[str, dict[str, Any]], str] | None = None, *, recon_config: ReconConfig | None = None, scope_gate: Any | None = None, risk_controller: Any | None = None, evidence_store: Any | None = None, blackboard: dict[str, Any] | None = None, model_client: Any = None, critic_agent: Any = None, reflection_agent: Any = None, experience_store: Any | None = None, semantic_memory: Any | None = None) -> None:
        ...
        if self._semantic_memory is None and bool(mission_config.get("semantic_memory", False)):
            try:
                ...
                self._semantic_memory = SemanticMemoryManager(db=get_default_db(), ollama_host=_embed_host, embedding_model=str(mission_config.get("embedding_model", "nomic-embed-text")))
            except _EXC_GROUP_CATCH as exc:
        ...
        self._executor = AttackModuleExecutor(scope_gate, risk_controller, evidence_store, blackboard=blackboard, mission_config=mission_config, model_client=model_client, critic_agent=critic_agent, reflection_agent=reflection_agent, tool_executor=tool_executor, opsec_manager=self._opsec, semantic_memory=self._semantic_memory, experience_store=self._experience_store)
        ...
        self._auto_local_exploit_suggester = bool(mission_config.get("msf_auto_les", False) or ((mission_config.get("msf") or {}).get("auto_local_exploit_suggester", False)))
    ...
    def _preflight_targets(self, targets: list[str]) -> list[str]:
        ...
        for target in targets:
            ...
            if not allowed:
                ...
                state.add_timeline_event("target_skipped_out_of_scope", f"Target {target} is not authorized: {reason}; skipping", {"target": target, "reason": reason})
            ...
            if self._skip_non_routable and is_private_or_local_target(effective):
                if not _is_local(effective):
                    state = self.get_state(target)
                    state.add_timeline_event("target_skipped_non_routable", f"Target {target} is non-routable ({effective}); skipping network campaign", {"target": target, "resolved_ip": effective or ""})
                    logger.info(f"[PREFLIGHT] {target} non-routable -- skipping")
                    continue
            dedup_key = effective if resolved else target
            if dedup_key in seen_ips:
                state = self.get_state(target)
                state.add_timeline_event("target_dedup", f"Target {target} resolves to {dedup_key}; already scheduled -- skipping duplicate", {"target": target, "resolved_ip": dedup_key})
    ...
    async def run_autonomous_campaign(self, targets: list[str], *, resume: bool = False, original_target: str = "", resolved_ip: str = "") -> dict[str, Any]:
        if original_target:
        ...
        return {"targets": targets, "results": results, "duration": campaign_duration, "total_tasks": len(self._tasks), "successful_exploits": sum(len(s.successful_exploits) for s in self._states.values()), "states": {t: s.to_dict() for t, s in self._states.items()}}

    def save_state(self, path: Path | None = None) -> Path:
        save_path = path or self._workspace / "attack_states.json"
        data = {"saved_at": datetime.now(timezone.utc).isoformat(), "states": {t: s.to_dict() for t, s in self._states.items()}, "tasks": {tid: t.to_dict() for tid, t in self._tasks.items()}, "task_counter": self._task_counter}
