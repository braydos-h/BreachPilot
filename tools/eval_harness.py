...
__all__ = [
    ...
    # reliability metrics, and live thresholds.
    "LiveOutcome",
    ...
    *,
    ...
    try:
        for oracle_file in sorted(oracle_dir.glob("*.oracle.json")):
            scenario_bits.append(f"{oracle_file.stem}:{oracle_file.stat().st_mtime_ns}")
...
class ReliabilityMetrics:
    ...
    fully-executed (non-skipped) targets unless noted.
    """
...
def compute_reliability_metrics(
    ...
    *,
    ...
    denom = len(executed)
    if denom <= 0:
        return ReliabilityMetrics(targets_skipped=int(skipped or 0), live_outcome=live_outcome)
