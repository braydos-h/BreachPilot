...
def _check_browser(config: dict[str, Any] | None = None) -> dict[str, Any]:
    """Browser-agent readiness (thin shim — see tools.browser.doctor_check).

    Extracted to keep this god-file from growing; the canonical contract
    (missing SDK = SKIP with install hint, not FAIL) lives in
    :func:`tools.browser.doctor_check.check_browser`.
    """
    from tools.browser.doctor_check import check_browser

    return check_browser(config)
...
def run_doctor(config_path: Path, json_output: bool = False) -> int:
    ...
    for c in checks:
        status = "SKIP" if c.get("skipped") else ("OK" if c.get("ok") else "FAIL")
        name = c.get("name", "unknown")
        ...
        if name == "browser":
            if c.get("note"):
                print(f"        -> {c['note']}")
            if c.get("skipped") and c.get("hint"):
                print(f"        -> {c['hint']}")
