...
- `chatgpt` (opt-in ChatGPT provider; `enabled`, `host`/`port`/`base_url` loopback `127.0.0.1:10531`, `auto_start`, `local_repo` `./openai-oauth`, `runtime` auto=bun-from-source, `default_model` `gpt-5.2`, `models` `[]`=discover from `/v1/models`, `context_window` 128000, login/start/discover timeouts, `oauth_file`) — active only when `models.provider: chatgpt`. Auth is browser OAuth via the vendored `openai-oauth/` proxy; tokens live in `~/.codex/auth.json` and are **never** copied into config or logged. Embeddings stay on Ollama under either provider.
