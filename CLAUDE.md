...
- `stealth` (rotate_ua, dns_over_https)
...
- **http**: starts the HTTP server, waits for the port, then wraps `mcp.client.streamable_http.streamable_http_client` and calls `session.initialize()` with the same 30s cap.
