...
- Lint / type-check are CI-enforced scoped: `ruff check app.py scope_gate.py tools/safety_reviewer.py tools/validation_utils.py tools/intelligence tools/providers` (scoped must pass, repo-wide `ruff check .` ≈27835 violations) and `mypy --follow-imports=skip summarizer.py planner.py observer.py target_graph.py outcome_judge.py db.py mcp_exploit_server.py tools/mcp_shared.py` (see `.github/workflows/ci.yml`). `pyproject.toml` has `ruff` line-length 120 `select = ["E","F","W","I"]` `ignore = ["E501"]` (`pyproject.toml:102-106`) and `mypy` configs. Keep security-sensitive diffs readable.
...
- **CI runs on every push/PR** (`.github/workflows/ci.yml` + codeql + dependency-review, `.github/dependabot.yml`): mocked test suite on Python 3.11-3.13 (`tests/: 248 files`), coverage (`Python 3.12`), scoped `ruff`/`mypy` (passing scopes in README §CI), package build (`python -m build` + `twine check`), WebUI build+tests (`npm ci` + `tsc -b && vite build` + `vitest`). Before a PR run `python -m pytest tests/ -v`, `ruff check app.py scope_gate.py tools/safety_reviewer.py tools/validation_utils.py tools/intelligence tools/providers`, and `mypy --follow-imports=skip` on the typed core, and verify README flags/config still match reality.
