...
def search_certificates(domain, include_expired=False):
    ...
    if not include_expired:
        ...
        certs = [
            cert
            for cert in certs
            if cert.get("not_after") and datetime.strptime(str(cert["not_after"]), "%Y-%m-%dT%H:%M:%S") > now
        ]
    return certs
...
def main():
    ...
    parser.add_argument(
        "--allowed-cas", nargs="*", default=["Let's Encrypt", "DigiCert", "Sectigo", "Amazon", "Google Trust Services"]
    )
    parser.add_argument("--output", default="ct_report.json")
    parser.add_argument(
        "--action", choices=["search", "typosquat", "unauthorized_ca", "monitor", "full_scan"], default="full_scan"
    )
