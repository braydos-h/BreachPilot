...
def test_stored_xss(base_url, submit_endpoint, display_endpoint, token, field="body"):
    ...
    for payload_type, payloads in XSS_PAYLOADS.items():
        for payload in payloads[:2]:
            ...
            try:
                resp = requests.post(
                    submit_url, headers=headers, json={field: tagged_payload}, timeout=10, verify=False
                )
...
def main():
    ...
    if args.submit_endpoint and args.display_endpoint:
        findings.extend(test_stored_xss(args.base_url, args.submit_endpoint, args.display_endpoint, args.token))
