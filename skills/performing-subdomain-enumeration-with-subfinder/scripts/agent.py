...
class SubdomainEnumerationAgent:
    ...
    def run_subfinder(self, all_sources=False, recursive=False):
        ...
        try:
            ...
            if out_file.exists():
                self.subdomains = [line.strip() for line in out_file.read_text().splitlines() if line.strip()]
            return {
    ...
    def detect_takeover_candidates(self):
        ...
        for record in dns_records:
            ...
            if any(cloud in cname for cloud in cloud_cnames):
                candidates.append(
                    {"subdomain": record.get("host", ""), "cname": cname, "risk": "Potential subdomain takeover"}
                )
        return candidates
