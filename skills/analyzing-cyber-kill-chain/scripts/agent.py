...
def map_event_to_phase(event_description):
    ...
    keyword_phase_map = {
        ...
        5: ["persist", "scheduled task", "registry", "run key", "service install", "web shell", "backdoor", "startup"],
        6: ["beacon", "c2", "c&c", "command and control", "callback", "dns tunnel"],
        7: ["exfiltrat", "lateral", "ransomware", "encrypt", "data stag", "credential dump", "mimikatz", "wiper"],
    }
...
def analyze_incident(events):
    ...
    analysis = {phase: {"events": [], "detected": False, "completed": False} for phase in range(1, 8)}
    for event in events:
