...
def calculate_criticality_score(asset):
    ...
    scores = {}
    scores["data_sensitivity"] = DATA_SENSITIVITY_SCORES.get(asset.get("data_classification", "internal").lower(), 2)
    scores["business_function"] = BUSINESS_FUNCTION_SCORES.get(
        asset.get("business_function", "internal-tool").lower(), 3
    )
    scores["regulatory_scope"] = REGULATORY_SCOPE_SCORES.get(asset.get("regulatory_scope", "none").lower(), 1)
    scores["network_exposure"] = NETWORK_EXPOSURE_SCORES.get(asset.get("network_exposure", "internal-only").lower(), 2)
    scores["recoverability"] = RECOVERABILITY_SCORES.get(asset.get("recoverability", "backup-available").lower(), 2)
...
def run_audit(args):
    ...
    if args.inventory:
        ...
        for s in scored[:20]:
            print(f"  Tier {s['tier']} ({s['tier_name']}): {s['asset']} — score {s['weighted_score']}")
        ...
        for tier in range(1, 6):
            ...
            print(
                f"  Tier {tier}: Critical={sla['critical']} High={sla['high']} Medium={sla['medium']} Low={sla['low']}"
            )
...
def main():
    ...
    parser.add_argument("--asset-tier", type=int, choices=[1, 2, 3, 4, 5], help="Asset criticality tier (1=highest)")
