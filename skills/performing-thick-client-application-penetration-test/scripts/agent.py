...
class ThickClientPentestAgent:
    ...
    def check_local_storage(self, app_name=None):
        ...
        sensitive_exts = {".db", ".sqlite", ".sqlite3", ".json", ".xml", ".config", ".ini", ".cfg", ".log"}
        ...
        for db_path in db_paths:
            try:
                ...
                for table in tables:
                    ...
                    if any(
                        kw in lower
                        for kw in ["user", "account", "credential", "auth", "login", "password", "token", "session"]
                    ):
                ...
                results.append({"database": db_path, "tables": tables, "sensitive_tables": sensitive_tables})
                conn.close()
            ...
            self.findings.append(
                {
                    ...
                    "severity": "High",
                    "details": f"Writable PATH dirs: {len(writable_dirs)}, App dir writable: {app_dir_writable}",
                }
            )
