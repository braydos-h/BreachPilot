...
def build_operation_plan(actor_name: str, target: str, objectives: List[str], attack_data: dict) -> RedTeamOperation:
...
def log_technique_execution(op: RedTeamOperation, technique_id: str, detected: bool = False, notes: str = "") -> None:
...
def generate_detection_gap_report(op: RedTeamOperation) -> dict:
    ...
    return {
        ...
        "recommendations": [f"Improve detection for {t.technique_id} ({t.technique_name})" for t in missed],
    }
