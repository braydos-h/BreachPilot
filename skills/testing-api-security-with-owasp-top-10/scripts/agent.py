...
def test_bola(base_url, token, endpoints, id_range=(1, 20)):
    ...
    for endpoint in endpoints:
        for obj_id in range(id_range[0], id_range[1]):
            ...
            try:
                resp = requests.get(
                    url, headers=headers, timeout=10, verify=not os.environ.get("SKIP_TLS_VERIFY", "").lower() == "true"
...
def test_security_headers(base_url):
    ...
    try:
        resp = requests.get(
            base_url, timeout=10, verify=not os.environ.get("SKIP_TLS_VERIFY", "").lower() == "true"
        )  # Set SKIP_TLS_VERIFY=true for self-signed certs in lab environments
        required_headers = {
...
def test_api_versions(base_url, path_prefix="/api"):
    ...
    for v in versions:
        ...
        try:
            resp = requests.get(
                url, timeout=5, verify=not os.environ.get("SKIP_TLS_VERIFY", "").lower() == "true"
            )  # Set SKIP_TLS_VERIFY=true for self-signed certs in lab environments
...
def main():
    ...
    if args.token:
        ...
        all_findings.extend(
            test_mass_assignment(args.base_url, args.token, args.endpoints[0], {"role": "admin", "is_admin": True})
        )
