...
def test_null_origin(url, cookies=None):
    ...
    try:
        ...
        if acao == "null":
            ...
            return [{"url": url, "issue": "null_origin_accepted", "credentials": creds, "severity": severity}]
        else:
...
def main():
    ...
    parser.add_argument("--endpoints", nargs="+", default=["/api/user/profile", "/api/users", "/api/account"])
