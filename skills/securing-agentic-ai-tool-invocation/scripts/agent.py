...
TOOL_SCHEMAS = {
    ...
    "transfer_funds": {
        "type": "object",
        "properties": {"amount": {"type": "number", "minimum": 0, "maximum": 1000}, "account": {"type": "string"}},
        "required": ["amount"],
        "additionalProperties": False,
    },
    "run_shell": {
...
def authorize(tool, args, actor):
    ...
    if policy["approval"]:
        return _event("require_approval", tool, args, actor, f"high-impact ({policy['impact']})")
...
def hitl_prompt(event, auto_approve):
    ...
    ans = input(f"APPROVAL: call {event['tool']} (args {event['args_sha256'][:12]})? [y/N] ").strip().lower()
    return ans == "y"
...
def main():
    ...
    ap.add_argument("--auto-approve", action="store_true", help="Auto-approve HITL (testing only)")
