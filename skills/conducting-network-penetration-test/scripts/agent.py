...
def port_scan(target, ports="1-10000", scan_type="-sS"):
    ...
    logger.info("Port scan: %d hosts, %d total services", len(results), sum(len(h["services"]) for h in results))
    return results
...
def smb_enumeration(target):
    ...
    scanner = nmap.PortScanner()
    scanner.scan(hosts=target, ports="139,445", arguments="--script=smb-enum-shares,smb-enum-users,smb-os-discovery")
    results = {}
    for host in scanner.all_hosts():
        for proto in scanner[host].all_protocols():
            for port in [139, 445]:
