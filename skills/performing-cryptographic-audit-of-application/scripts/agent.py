...
def scan_directory(directory, extensions=None, exclude_dirs=None):
    ...
    if exclude_dirs is None:
        exclude_dirs = {"node_modules", ".git", "__pycache__", "venv", ".venv", "vendor", "dist", "build", ".tox"}
...
def audit_tls_endpoint(host, port=443):
    ...
    try:
                ...
                findings.append(
                    {
                        "check": "TLS Protocol",
                        "value": protocol,
                        "severity": "CRITICAL" if "TLSv1.0" in protocol or "TLSv1.1" in protocol else "INFO",
                    ...
                    findings.append(
                        {
                            ...
                            "value": cipher_name,
                            "severity": "HIGH"
                            if any(w in cipher_name for w in ["RC4", "DES", "NULL", "EXPORT"])
                            else "INFO",
                    ...
                    findings.append(
                        {
                            ...
                            "severity": "CRITICAL" if key_size < 2048 else "MEDIUM" if key_size < 4096 else "INFO",
                        }
                    )
                    findings.append(
                        {
        ...
        findings.append({"check": "TLS Connection", "severity": "CRITICAL", "description": f"SSL error: {e}"})
    except socket.timeout:
        findings.append({"check": "TLS Connection", "severity": "HIGH", "description": "Connection timed out"})
    except Exception as e:
        findings.append({"check": "TLS Connection", "severity": "HIGH", "description": f"Error: {e}"})
...
def format_summary(code_findings, tls_findings, files_scanned, target):
    ...
    if tls_findings:
        ...
        for f in tls_findings:
            print(f"    [{f['severity']:8s}] {f.get('check', '')}: {f.get('value', f.get('description', ''))}")

    return severity_counts


def main():
    parser = argparse.ArgumentParser(description="Application cryptographic audit agent")
    parser.add_argument("--target", required=True, help="Source directory to scan or TLS host to audit")
    parser.add_argument("--tls-port", type=int, default=443, help="TLS port for endpoint audit (default: 443)")
    parser.add_argument("--tls-only", action="store_true", help="Only audit TLS endpoint, skip code scan")
    parser.add_argument("--code-only", action="store_true", help="Only scan code, skip TLS audit")
    parser.add_argument("--exclude-dirs", nargs="+", help="Additional directories to exclude from scan")
