...
class BloodHoundAnalyzer:
    ...
    def find_unconstrained_delegation(self) -> list[Finding]:
        ...
        unconstrained = [
            ...
            if c.unconstrained_delegation and c.enabled and "DOMAIN CONTROLLER" not in c.name.upper()
        ]
        findings = []
        ...
        if constrained:
            findings.append(
                Finding(
                    ...
                    affected_objects=[f"{c.name} -> {', '.join(c.allowed_to_delegate)}" for c in constrained],
                    remediation=(
...
def main():
    ...
    sample_groups = {
        ...
        "data": [
            ...
            {"Properties": {"name": "BACKUP OPERATORS@CORP.LOCAL", "highvalue": True}, "Members": []},
        ],
    }
