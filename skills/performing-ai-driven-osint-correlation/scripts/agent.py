...
def generate_markdown_report(report, output_path):
    ...
    for entity in report.get("entities", [])[:50]:
        ...
        for link in entity.get("linked_accounts", []):
            md += f"| {link.get('source', '')} | {link.get('platform', '')} | {link.get('value', '')} |\n"
...
def main():
    ...
    parser.add_argument("--target", default="unknown", help="Target identifier (domain, username, etc.)")
    ...
    parser.add_argument("--normalize-only", action="store_true", help="Only normalize data, skip correlation")
    ...
    print(f"[*] Loading data from {len(active_sources)} source(s): {', '.join(active_sources.keys())}")
