...
def enumerate_subdomains(domain: str, output_dir: Path) -> list[str]:
    ...
    stdout, stderr, rc = run_command(["subfinder", "-d", domain, "-silent", "-o", str(subfinder_out)], timeout=600)
...
def nmap_scan(targets: str, output_dir: Path, scan_type: str = "quick") -> dict:
    ...
    if scan_type == "quick":
        ...
        cmd = ["nmap", "-sS", "-sV", "-p-", "-T4", "--min-rate", "1000", "-oA", output_prefix, targets]
    elif scan_type == "udp":
...
def generate_report(scan_results: dict, resolved: dict, ssl_results: list[dict], output_dir: Path) -> str:
...
def main():
    ...
    parser.add_argument(
        "--scan-type", default="quick", choices=["quick", "full", "udp", "scripts"], help="Nmap scan type"
    )
