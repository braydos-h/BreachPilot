...
def audit_execution_role(iam, role_arn):
    ...
    try:
        ...
        for policy in attached["AttachedPolicies"]:
            ...
            doc = iam.get_policy_version(PolicyArn=policy["PolicyArn"], VersionId=version_id)["PolicyVersion"][
                "Document"
            ]
            for stmt in doc.get("Statement", []):
                ...
                if "*" in actions or any(a.endswith(":*") for a in actions):
                    findings.append(
                        f"WARNING: {role_name} has wildcard action: {actions} on {stmt.get('Resource', '*')}"
                    )
    ...
    try:
        ...
        for stmt in doc.get("Statement", []):
            ...
            if principal == "*" or principal == {"AWS": "*"}:
                findings.append(
                    f"PUBLIC ACCESS: {function_name} allows public invocation (statement: {stmt.get('Sid', 'unnamed')})"
                )
    except ClientError:
        pass
    try:
