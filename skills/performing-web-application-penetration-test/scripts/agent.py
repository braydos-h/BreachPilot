...
def test_cors_config(target_url):
    ...
    for origin in origins:
        try:
            resp = requests.get(target_url, headers={"Origin": origin}, timeout=5, verify=False)
...
def run_assessment(target_url, test_params=None):
    ...
    missing_headers = [
        h for h, v in report["security_headers"].items() if isinstance(v, dict) and not v.get("present", True)
    ]
    report["summary"] = {
