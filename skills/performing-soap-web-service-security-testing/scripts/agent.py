...
class SOAPSecurityTester:
            ...
            try:
                ...
                if vulnerable:
                    self.findings.append(
                        {"severity": "CRITICAL", "type": "XXE", "operation": operation_name, "test": name}
                    )
                results.append(
                    {
                        "test": name,
    ...
    def test_sql_injection(self, operation_name, soap_action=""):
        ...
        sqli_payloads = ["' OR '1'='1", "1; DROP TABLE users--", "' UNION SELECT NULL--", "admin'/*"]
        results = []
        sql_errors = ["SQL syntax", "ORA-", "SQLSTATE", "Unclosed quotation", "Microsoft OLE DB", "PostgreSQL"]
        for payload in sqli_payloads:
            ...
            try:
                ...
                results.append({"payload": payload, "sql_error": error_found, "status": resp.status_code})
            ...
            for j, other in enumerate(self.operations):
                ...
                try:
                    ...
                    if resp.status_code == 200 and "Fault" not in resp.text:
                        ...
                        results.append({"op": op["name"], "spoofed": other["action"], "accepted": True})
                except requests.RequestException:
                    continue
        return results
    ...
    def test_ws_security_bypass(self):
        ...
        try:
            ...
            if accepted:
                self.findings.append({"severity": "CRITICAL", "type": "WS-Security Bypass", "operation": op["name"]})
