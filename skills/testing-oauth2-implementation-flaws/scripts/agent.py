...
class OAuth2TestAgent:
    ...
    def test_scope_escalation(self, extra_scopes=None):
        ...
        for scope in scopes:
            ...
            if resp and resp.status_code in (301, 302, 303, 307):
                ...
                self.findings.append(
                    {"severity": "high", "type": "Scope Escalation", "detail": f"Server granted scope: {scope}"}
                )
        return results
    ...
    def test_code_reuse(self, auth_code):
        ...
        if resp2 and resp2.status_code == 200:
            self.findings.append(
                {"severity": "high", "type": "Code Reuse", "detail": "Authorization code accepted multiple times"}
            )
