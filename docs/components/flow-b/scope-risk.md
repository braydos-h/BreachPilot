...
- Provide rule engine: `_rule_matches` (domain exact, wildcard `*.`, ip, cidr `subnet_of`, url_prefix) (`scope_gate.py:346`), `_classify_target_type`, `_is_third_party_asset` (anchored regex + explicit cdn checks, `scope_gate.py:436`), `_clean_asset` (IPv6-safe, CIDR-preserving, `scope_gate.py:455`).
