...
| Exploit agent entry | `tools/exploit_agent/runner/_impl.py (per-round LLM call)` | the main `await _call_ollama_with_retry(client, model, ...)` |
...
- `tools/exploit_agent/runner/_impl.py (embed host resolution)` (Flow A) — same pattern
...
| `embed_host` | `http://localhost:11434` | `config_manager.py:998-1006`, `tools/exploit_agent/runner/_impl.py (embed host resolution)` |
...
(`agent_loop.py:176-182`, `tools/exploit_agent/runner/_impl.py (embed host resolution)`,
...
- **Two flows share `db.py`/`mission.py` schemas only.** Flow A
  ...
  (`agent_loop.py:176-182` vs `tools/exploit_agent/runner/_impl.py (embed host resolution)`). An embeddings
