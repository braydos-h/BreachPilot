...
| `--version` | — | `store version` | n/a | — | `tools/cli_args.py:46` `argparse version` | `python main.py --version` | — | 0 + print `BreachPilot <ver>` |
| `--target` | — | `str` | `""` | daemon/web | `tools/cli_args.py:49` core targeting; threaded via `mcp_session.py:255` → `EXPLOIT_TARGET*` env; `_allowed_target_list` union; persisted via `config_cli.add_target_to_allowlist` on interactive menu | `python main.py --target 10.0.0.50 --mode attack --goal backdoor`<br>`python main.py --target example.com --mode recon` | `exploit.allowed_targets` (union), `exploit.require_explicit_allowlist` | 1 if malformed (not IP/FQDN) |
| `--mode` | — | `choices: recon \| attack \| fast` | `""` → `recon` in `RunRequest` | — | `tools/cli_args.py:50` core | `python main.py --target 10.0.0.50 --mode recon --goal initial_access`<br>`--mode fast` = parallel recon preset then attack | `exploit.*`, `reasoning.*` budgets; recon always `READ_ONLY` via `cli_exploit_settings:157` | — |
| `--goal` | — | `str` | `""` | — | `tools/cli_args.py:56` core → `GoalEngine` | `python main.py --target 10.0.0.50 --goal backdoor` | — | 1 if preset unknown (via `GoalEngine`) |
| `--custom-goal` | — | `str` | `""` | — | `tools/cli_args.py:59` core | `python main.py --target 10.0.0.50 --custom-goal \"get domain admin\"` | — | — |
| `--config` | — | `Path` | `Path("config.yaml")` | — | `tools/cli_args.py:60` all config loads (`tools/kernel/config.load_config`, `ConfigValidator`, `load_validated_config`, `doctor`, `self_test`, `app.create_app`) | `python main.py --config ./my.yaml --target 10.0.0.50` | — (path to file) | 1 if non-mapping YAML |
| `--model` | — | `str` \| `None` | `None` → `models.default_alias` | — | `tools/cli_args.py:61` core; `run_service/service.py:349` model alias resolve | `python main.py --target 10.0.0.50 --model kimi` | `models.default_alias`, `models.registry` | — |
| `--model-strategy` | — | `choices: default \| round-robin \| random \| specific` | `"default"` | — | `tools/cli_args.py:64` core (passed to orchestrator campaign) | `python main.py --target 10.0.0.50 --model-strategy random` | — | — |
| `--mcp-transport` | — | `choices: stdio \| http` \| `None` | `None` | — | `tools/cli_args.py:70` core; **ignored on run path** — always forced to `http` so target-IP lock reaches server (`mcp_session.py`) | `python main.py --mcp-transport http --http-port 8001 --target 10.0.0.50` | `mcp.default_transport` (default `stdio`) | — |
| `--http-port` | — | `int` \| `None` | `None` | — | `tools/cli_args.py:76` core → `mcp_session` HTTP port | `python main.py --http-port 9001 --target 10.0.0.50` | `mcp.http_port` (default 8001) | — |
| `--reports-dir` | — | `Path` | `Path("reports")` | — | `tools/cli_args.py:77` core; `RunRequest.reports_dir` → `reports/<run_id>/` | `python main.py --reports-dir ./my_reports --target 10.0.0.50` | `reports_dir` (top-level override) | — |
| `--setup-api-keys` | — | `store_true` | `False` | daemon/web | `tools/cli_args.py:82` keys group → `bootstrap_startup_api_keys(force_prompt=True)` → `api_key_store.save_api_keys` | `python main.py --setup-api-keys --api-key-file secr.json` | `ollama.api_key_env`, `research.*`, `cve_lookup.*` (via `configured_api_key_env_names`) | 0 solo; prompts then saves `secr.json` `0o600` |
| `--api-key-file` | — | `Path` | `Path("secr.json")` (`DEFAULT_API_KEY_FILE`) | — | `tools/cli_args.py:85` keys group | `python main.py --api-key-file ./secrets.json --target 10.0.0.50` | — | — |
| `--no-api-key-prompt` | — | `store_true` | `False` | — | `tools/cli_args.py:88` keys group → suppresses `bootstrap_startup_api_keys(prompt=...)` | `python main.py --no-api-key-prompt --target 10.0.0.50` | — | — |
| `--plain` | — | `store_true` | `False` | — | `tools/cli_args.py:91` output → `ui.plain = plain or quiet or json` | `python main.py --plain --target 10.0.0.50` | — | — |
| `--menu` | — | `store_true` | `False` | daemon/web | `tools/cli_args.py:92` output → forces `interactive_menu` even with args; also flips `interactive_startup` for api-key prompt | `python main.py --menu` | — | — |
| `--json` | — | `store_true` | `False` | — | `tools/cli_args.py:93` output → machine-readable + forces plain | `python main.py --json --target 10.0.0.50 --mode recon` | — | — |
| `--quiet` | — | `store_true` | `False` | — | `tools/cli_args.py:94` output → warnings/errors only + forces plain | `python main.py --quiet --target 10.0.0.50` | — | — |
| `--debug` | — | `store_true` | `False` | — | `tools/cli_args.py:95` output → `os.environ["AI_NMAP_DEBUG"]=1` (`main.py:590`), verbose logging | `python main.py --debug --target 10.0.0.50` | — | — |
| `--swarm` | — | `store_true` | `False` | — | `tools/cli_args.py:98` swarm → `RunRequest.swarm` → `AgentLoop.run_autonomous_campaign` / `SwarmOrchestrator` | `python main.py --target 10.0.0.50 --mode attack --swarm` | `swarm.enabled` | — |
| `--parallel-swarm` | — | `store_true` | `False` | — | `tools/cli_args.py:106` swarm → flips `swarm.parallel_enabled` to true; gates `route_parallel` + `spawn_subagent` MCP tool | `python main.py --target 10.0.0.50 --swarm --parallel-swarm` | `swarm.parallel_enabled`, `swarm.per_phase_concurrency`, `swarm.exploit_parallel`, `swarm.subagent_timeout_seconds` | — |
| `--critic` | — | `store_true` | `False` | — (requires `--swarm` at run time; flag alone no-op) | `tools/cli_args.py:114` swarm | `python main.py --target 10.0.0.50 --swarm --critic` | `swarm.critic_enabled` (via `cli_exploit_settings`) | — |
| `--reflection` | — | `store_true` | `False` | — (requires `--swarm`) | `tools/cli_args.py:115` swarm | `python main.py --target 10.0.0.50 --swarm --reflection` | `swarm.reflection_enabled` | — |
| `--adaptive-exploits` | — | `store_true` | `False` | — | `tools/cli_args.py:116` swarm → `adaptive_exploits.enabled` | `python main.py --target 10.0.0.50 --adaptive-exploits` | `adaptive_exploits.enabled`, `max_mutations`, `mutation_strategies` | — |
| `--long-session` | — | `store_true` (`dest long_session`) | `False` | — | `tools/cli_args.py:119` swarm → raises `long_session.*` budgets (`cli_exploit_settings:43`, `_compute_swarm_timeout`) + `request_timeout_seconds` | `python main.py --target 10.0.0.50 --mode attack --long-session` | `long_session.enabled`, `request_timeout_seconds`, `attack_max_*`, `swarm_session_timeout_minutes`, `persist_messages` | — |
| `--multi-model-consult` | — | `store_true` (`dest multi_model_consult`, `default None`) | `None` → `multi_model.enabled` | mutually flips same dest with `--no-multi-model-consult` | `tools/cli_args.py:126` swarm → `RunRequest.multi_model_consult`; `main.py:607` default → `multi_model.enabled`; also injected as `AI_NMAP_MULTI_MODEL_ENABLED` | `python main.py --target 10.0.0.50 --multi-model-consult` | `multi_model.enabled`, `consult_aliases`, `max_consultations`, `max_question_chars` | — |
| `--no-multi-model-consult` | — | `store_false` (same dest) | `None` | same dest | `tools/cli_args.py:133` swarm | `python main.py --target 10.0.0.50 --no-multi-model-consult` | same | — |
| `--observer-mode` | — | `choices: heuristic \| llm \| hybrid` | `"hybrid"` | — | `tools/cli_args.py:139` swarm → `RunRequest.observer_mode` → `cli_exploit_settings` / `exploit_agent` | `python main.py --target 10.0.0.50 --observer-mode llm` | `reasoning.observer_mode` | — |
| `--recon-first` | — | `store_true` (`dest recon_first`, `default None`) | `None` | mutual with `--no-recon-first` | `tools/cli_args.py:145` swarm → `RunRequest.recon_first` → `async_main` / `run_service` goal suggestion | `python main.py --target 10.0.0.50 --recon-first` | — | — |
| `--no-recon-first` | — | `store_false` (same dest) | `None` | same | `tools/cli_args.py:151` swarm | `python main.py --target 10.0.0.50 --no-recon-first --goal backdoor` | — | — |
| `--ultrathink` | — | `store_true` | `False` | — | `tools/cli_args.py:157` swarm → `RunRequest.ultrathink` → `cli_exploit_settings.chain_of_thought` + `verbose_reasoning`; `main.py:594` banner | `python main.py --target 10.0.0.50 --ultrathink` | `reasoning.ultrathink`, `ultrathink_reflection_interval`, `llm_reflection` | — |
| `--doctor` | — | `store_true` | `False` | daemon/web | `tools/cli_args.py:164` ops → `tools/doctor.run_doctor(config_path)` → exits before exploit session | `python main.py --doctor` | `ollama.host`, `models.registry`, `nmap.path`, `mcp.http_port`, `api.host` | 0 all pass; 1 any fail |
| `--demo` | — | `store_true` | `False` | daemon/web | `tools/cli_args.py:165` ops → `tools/demo_mode.run_demo` (Docker DVWA or synthetic server) | `python main.py --demo` | — | — |
| `--resume` | — | `str` | `""` | — | `tools/cli_args.py:166` ops → `RunRequest.resume_source` → `tools/resume_state` + `RunPreview` | `python main.py --resume <run_id>` | `long_session.persist_messages` (`session_state.json`) | — |
| `--export-run` | — | `str` | `""` | — | `tools/cli_args.py:167` | — | — | — |
| `--yes` | — | `store_true` | `False` | — | `tools/cli_args.py:174` ops → skips ready-to-begin confirmation gate (`main.py:1094`); destructive runs need matching confirmation text instead of `Y` | `python main.py --target 10.0.0.50 --mode attack --yes` | — | — (careful) |
| `--self-test` | — | `store_true` | `False` | daemon/web (also blocks auto ChatGPT runtime ensure) | `tools/cli_args.py:175` ops → `tools/self_test.run_self_test` (localhost-only, `read_only`, `check_os/quick_scan/search_cve_intel/list_workspace`) → `reports/self_test_<run_id>/` | `python main.py --self-test` | `exploit.permission` forced `read_only` | 0 pass; 1 fail; rejects non-127.0.0.1 |
| `--eval` | — | `nargs="*"` (`metavar TARGET`) | `None` (bare `--eval` = `[]` → all graded targets) | daemon/web | `tools/cli_args.py:179` eval group → `run_graded_eval` (no `--target`) or legacy `run_eval` (with `--target`) | `python main.py --eval`<br>`python main.py --eval dvwa juice_shop`<br>`python main.py --eval --target 10.0.0.50` (legacy single-target) | `eval.*` (output_dir, max_rounds, baseline_path) | graded 0 (1 on `--check-regression` fail); legacy writes `reports/eval/<run_id>/` |
| `--eval-list` | — | `store_true` (`dest eval_list`) | n/a | daemon/web | `tools/cli_args.py:188` | `python main.py --eval-list` | `eval_targets/*.oracle.json` | 0 + stdout |
| `--save-baseline` | — | `store_true` (`dest save_baseline`) | n/a | requires `--eval`/`--benchmark` → 2 | `tools/cli_args.py:194` | `python main.py --eval --save-baseline` | `eval.baseline_path` | 0 + writes baseline |
| `--check-regression` | — | `store_true` (`dest check_regression`) | n/a | requires `--eval`/`--benchmark` → 2 | `tools/cli_args.py:200` | `python main.py --eval --check-regression` | `eval.baseline_path`, `eval.regression_tolerance` | 1 on hard regression |
| `--benchmark` | — | `nargs="*"` (`metavar SUITE`) \| `None` | `None` | daemon/web | `tools/cli_args.py:207` | `python main.py --benchmark xben` | `benchmark.*` | via `run_benchmark_cli` |
| `--benchmark-list` | — | `store_true` (`dest benchmark_list`) | n/a | daemon/web | `tools/cli_args.py:215` | `python main.py --benchmark-list` | `benchmark.*` suites | 0 + stdout |
