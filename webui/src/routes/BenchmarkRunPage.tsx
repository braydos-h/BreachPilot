...
import { cancelBenchmarkRun, fetchOverview, fetchRun, fetchRunEvents, saveBaseline } from "@/features/benchmarks/api";
...
export function BenchmarkRunPage() {
  ...
  const isActiveRun =
    (overview.data?.active.run_id === runId && isRunActive(overview.data.active.state)) ||
    (run.data ? isRunActive(run.data.status) : false);
  ...
  const elapsedSec = (events.data as { events: BenchmarkEvent[] } | undefined)?.events?.length
    ? ((events.data as { events: BenchmarkEvent[] }).events[(events.data as { events: BenchmarkEvent[] }).events.length - 1]?.elapsed_seconds ?? 0)
    : 0;
  const eventCount = (events.data as { events: BenchmarkEvent[] } | undefined)?.events?.length ?? 0;
  const trialIds = [...new Set(data.trials.map((t) => t.trial_id))];
      ...
      <div className="rounded-md border bg-card/50 px-2.5 py-2">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span className="text-xs text-muted-foreground">
            {data.suite} · {data.trials[0]?.started_at ? formatRelative(data.trials[0].started_at) : "new run"}
          </span>
          {isActiveRun && (
          ...
          <span className="ml-auto flex items-center gap-2">
            {isActiveRun ? (
              <Button size="sm" variant="destructive" className="h-7" onClick={onCancel}>
                <XCircle className="h-3.5 w-3.5" />
                Cancel
              </Button>
            ) : summary ? (
              <Button size="sm" variant="outline" className="h-7" onClick={onSaveBaseline}>
                Save baseline
              </Button>
            ) : null}
          </span>
                ...
                <Card>
                  ...
                  <CardContent className="space-y-2">
                    <BenchmarkTimeline
                      ...
                      maxEvents={400}
                    />
                    {events.isFetching && (events.data as { events: BenchmarkEvent[] } | undefined)?.events?.length ? (
                ...
                <Card>
                  ...
                  <CardContent className="space-y-3">
                    {data.trials.length === 0 ? (
                      <div className="py-6 text-center text-sm text-muted-foreground">No trials yet.</div>
                    ) : (
                      data.trials.map((t) => (
                        <div key={t.trial_id} className="rounded-md border p-3">
                          ...
                          {t.flags.length > 0 && (
                            <div className="mt-2 flex flex-wrap gap-1.5">
                              {t.flags.map((f) => (
                                <Badge
                                  key={String(f.flag_id ?? Math.random())}
        ...
        <aside className="flex min-w-0 shrink-0 flex-col gap-2 xl:w-[320px] xl:min-h-0 xl:overflow-y-auto xl:overflow-x-hidden scrollbar-thin">
          <Card>
            <CardHeader className="pb-2">
              ...
              <CardDescription>
                {isActiveRun ? "Live until terminal" : "Frozen at completion"}
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-2 text-sm">
              <div className="grid grid-cols-2 gap-2">
                ...
                <div className="rounded-md bg-muted/40 p-2">
                  ...
                  <div className="font-medium tabular-nums">{summary ? summary.total_tokens.toLocaleString() : data.trials.reduce((a, t) => a + (t.total_tokens ?? 0), 0).toLocaleString()}</div>
                ...
                <div className="rounded-md bg-muted/40 p-2">
                  ...
                  <div className="font-medium tabular-nums">{summary ? `${summary.solved}/${summary.trials_total}` : `${data.trials.filter((t) => t.oracle_verified_success).length}/${data.trials.length}`}</div>
              ...
              {!summary && isActiveRun && data.trials.length > 0 && (
                <div className="space-y-1">
                  <div className="text-xs text-muted-foreground">Current trials</div>
                  {data.trials.slice(0, 4).map((t) => (
                  ...
                  {data.trials.length > 4 && <div className="text-center text-[11px] text-muted-foreground">+{data.trials.length - 4} more</div>}
                </div>
              )}
            </CardContent>
          </Card>
