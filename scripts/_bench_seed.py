...
def make_trial(run_id: str, scenario: str, idx: int, verified: bool, fp: bool, duration: float) -> TrialResult:
    ...
    t.status = TrialStatus.VERIFIED.value if verified else (TrialStatus.FALSE_POSITIVE.value if fp else TrialStatus.FAILED.value)
...
def seed(run_id: str, scenarios: list[str], verified_flags: list[bool], fp_flags: list[bool], stamp: str) -> None:
    ...
    (STORAGE.run_dir("xben", run_id) / "events.jsonl").write_text(
        "\n".join(
            [
                '{"sequence":1,"timestamp":"' + stamp + '","elapsed_seconds":0.0,"run_id":"' + run_id + '","type":"run_start","level":"info","trial_id":"","scenario_id":"","agent":"","tool":"","target":"","payload":{"suite":"xben"}}',
                '{"sequence":2,"timestamp":"' + stamp + '","elapsed_seconds":12.5,"run_id":"' + run_id + '","type":"target_ready","level":"info","trial_id":"' + scenarios[0] + '#t1","scenario_id":"' + scenarios[0] + '","agent":"","tool":"","target":"127.0.0.1","payload":{"host":"127.0.0.1","ports":[8080]}}',
                '{"sequence":3,"timestamp":"' + stamp + '","elapsed_seconds":420.0,"run_id":"' + run_id + '","type":"oracle_result","level":"info","trial_id":"' + scenarios[0] + '#t1","scenario_id":"' + scenarios[0] + '","agent":"","tool":"","target":"127.0.0.1","payload":{"verified":' + ("true" if verified_flags[0] else "false") + ',"flags_captured":1,"flags_total":1}}',
            ]
        )
        + "\n",
        encoding="utf-8",
...
seed("20260829_120000_00001", ["xben-dvwa", "xben-juice-shop", "xben-metasploitable2", "xben-vulnerable-k8s"],
     [True, False, True, False], [False, False, False, False], "2026-08-29T12:05:00+00:00")
seed("20260830_120000_00002", ["xben-dvwa", "xben-juice-shop", "xben-metasploitable2", "xben-vulnerable-k8s"],
     [True, True, True, False], [False, False, False, False], "2026-08-30T12:05:00+00:00")
