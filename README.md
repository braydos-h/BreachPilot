...
| Structured offensive tools | First-class MCP tools that wrap Kali scanners and crackers so the AI gets parsed output and a consistent audit record instead of raw `run_exploit_terminal` shell dumps: `run_web_scan` (nikto/nuclei/sqlmap/gobuster/feroxbuster/whatweb/wpscan/dirb/dirbuster — target-IP allowlist-locked) and `run_hash_crack` (hashcat/john, local-only with auto hash-type identification and `--show` plaintext recovery). |
