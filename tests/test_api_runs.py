...
def test_second_run_returns_409(tmp_path, monkeypatch):
    ...
    resp = client.post("/api/v1/runs", json={
        "target": "10.0.0.51", "mode": "attack", "goal": "recon_only",
    }, headers=_auth_headers())
...
def test_answer_start_confirmation_schedules_execution(tmp_path, monkeypatch):
    ...
    run = client.get(
        f"/api/v1/runs/{created['run_id']}", headers=_auth_headers(),
    ).json()
...
def test_yes_starts_without_a_decision(tmp_path, monkeypatch):
    ...
    response = client.post("/api/v1/runs", json={
        "target": "10.0.0.50", "mode": "attack",
        "goal": "recon_only", "yes": True,
    }, headers=_auth_headers())
...
def test_cancel_before_start_releases_active_slot(tmp_path, monkeypatch):
    ...
    cancelled = client.post(
        f"/api/v1/runs/{created['run_id']}/cancel", headers=_auth_headers(),
    )
...
def test_resume_persists_parent_and_request_flags(tmp_path, monkeypatch):
    client = _make_client(tmp_path, monkeypatch)
    original = client.post("/api/v1/runs", json={
        "target": "10.0.0.50", "mode": "attack",
        "goal": "recon_only", "critic": True,
    }, headers=_auth_headers()).json()
    client.post(
        f"/api/v1/runs/{original['run_id']}/cancel", headers=_auth_headers(),
    )
    resumed = client.post(
        f"/api/v1/runs/{original['run_id']}/resume", headers=_auth_headers(),
    )
    assert resumed.status_code == 200
    run = client.get(
        f"/api/v1/runs/{resumed.json()['run_id']}", headers=_auth_headers(),
    ).json()
