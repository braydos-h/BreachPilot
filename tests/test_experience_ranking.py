...
class FakeExperience:
    ...
    def __init__(self, confs: dict[str, dict[str, float]] | None = None, raises: bool = False) -> None:
...
def test_find_modules_demotes_failing_module_below_untried():
    ...
    untried = [(m, s) for m, s in static.items() if m != "SSHBruteForce" and s == ssh_baseline]
    if untried:
...
def test_find_modules_neutral_when_no_recorded_data():
    ...
    for name, baseline in static.items():
        assert with_store[name] == pytest.approx(baseline), f"{name} shifted despite no recorded data"
...
class _RecordingMutator:
    ...
    def craft_initial(self, *, target_ip, service_name, version, os_hint, module_name):
        self.captured = {
...
def test_read_write_signature_coherence_for_multi_service_module():
    ...
    assert read_sig == write_sig, f"read/write signature divergence: read={read_sig!r} write={write_sig!r}"
...
def test_multi_service_module_experience_applies_when_recon_reports_present_service():
    ...
    assert write_sig in exp.queries, f"read side queried {exp.queries!r}; never the write-side sig {write_sig!r}"
...
def test_vuln_agent_passes_experience_store_to_find_modules(monkeypatch):
    ...
    context = {"config": {}, "model_client": None, "blackboard": {}, "experience": fake_store}
...
async def test_run_exploit_agent_uses_provided_experience_store(monkeypatch, tmp_path):
    ...
    class RecordingStore:
        ...
        def get_confidence(self, sig, action):
            return 0.5

        def update_from_exploit_result(self, **kw):
            pass

        def update_from_result(self, *a, **k):
            pass
    ...
    assert mutator_stores[0] is provided, "run_exploit_agent rebuilt its own store instead of using the provided one"
    # ...and get_default_db was NOT called for the experience store rebuild.
    assert not gdb_calls, "run_exploit_agent called get_default_db despite a provided experience_store"
...
def _insert_lessons_row(conn, *, target_signature, action_type, outcome, embedding_json, confidence=0.5):
...
def test_get_all_confidences_excludes_distilled_lesson_rows(exp_db):
    ...
    assert all_conf["EternalBlue"] == pytest.approx(0.75, abs=0.01), (
        f"lesson row leaked into get_all_confidences: got {all_conf['EternalBlue']}, expected ~0.75"
    )
