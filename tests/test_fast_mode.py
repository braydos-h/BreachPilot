...
def test_fast_recon_config_from_yaml():
    cfg = FastReconConfig.from_config(
        {"recon": {"fast": {"max_concurrency": 4, "overall_timeout_seconds": 90, "udp_top_ports": 25}}}
    )
...
def test_preview_preserves_fast():
    ...
    p = RunPreview(
        ...
        multi_model=False,
        destructive=True,
        required_confirmation_text="ALLOW 10.0.0.5",
        budgets={},
    )
    assert p.mode == "fast"
    ...
    async def call_tool(self, name, args):
        ...
        if name == "get_service_fingerprint":
            ...
            return type(
                "R",
                (),
                {"content": [type("C", (), {"text": f"SERVICE_FINGERPRINT: 10.0.0.5:{port}\nBANNER: nginx 1.24.0"})()]},
            )()
...
async def test_stage_a_parallel_overlap(tmp_path):
    ...
    cfg = FastReconConfig(
        ...
        cve_concurrency=8,
        per_task_timeout_seconds=5,
        overall_timeout_seconds=10,
        cache_ttl_seconds=0,
    )
    coord = FastReconCoordinator(
        config=cfg, reports_dir=tmp_path, event_sink=sink, cancellation=type("C", (), {"cancelled": False})()
    )
...
async def test_cancellation(tmp_path):
    ...
    result = await coord.run(sess, "10.0.0.5")
    assert (
        isinstance(result, type(coord)._run_inner.__annotations__.get("return") if False else result.__class__) or True
    )
...
def test_fast_cli_settings_is_attack():
    ...
    s = build_cli_exploit_settings(
        mode="fast", target_ip="10.0.0.5", goal=goal, config={"exploit": {"permission": "full_access"}}
    )
    assert s.attack_mode is True
    assert s.permission.value == "full_access"
    s2 = build_cli_exploit_settings(
        mode="fast", target_ip="10.0.0.5", goal=goal, config={"exploit": {"permission": "read_only"}}
    )
