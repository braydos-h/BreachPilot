...
def test_nmap_p_portspec_not_captured_as_target() -> None:
    ...
    cmd = (
        "nmap -p 445,139,135,389 --script smb-vuln-ms17-010 -Pn --open "
        "209.38.90.131"
    )
...
def test_fqdn_target_after_value_flag_is_still_checked() -> None:
    ...
    positional)."""
    assert _extract_scanner_targets("nmap -p 445 evil.example.com") == [
        "evil.example.com"
    ]
...
def test_attached_value_flags_not_split() -> None:
    ...
    assert _extract_scanner_targets("nmap --script=smb-vuln 10.0.0.5") == [
        "10.0.0.5"
    ]
...
def test_full_path_scanner_basename_recognized() -> None:
    ...
    assert _extract_scanner_targets("/usr/bin/nmap -p 445 10.0.0.5") == [
        "10.0.0.5"
    ]
...
def test_stops_at_shell_separator() -> None:
    ...
    targets."""
    assert _extract_scanner_targets("nmap 10.0.0.5 | grep open") == [
        "10.0.0.5"
    ]
