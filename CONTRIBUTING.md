...
- **Mypy**: `mypy --follow-imports=skip tools` must pass. Masks at `pyproject.toml:156`; 3 hot files strict (`tools/validation_utils.py`, `tools/exceptions.py`, `tools/mcp_shared.py` — zero disables, `pyproject.toml:201`). Raw debt without masks is tracked in `mypy-baseline.txt` (ratchet: never up).
